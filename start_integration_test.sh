#!/usr/bin/env bash
# Start a two-shard DatoriumDB Compose stack, run the Todo integration CLI
# through the Go client library, then tear the stack down.
#
# Step titles match tech-docs/TESTING.md ("Todo integration story").
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
COMPOSE_DIR="${ROOT}/test/integration/todo"
COMPOSE_FILE="${COMPOSE_DIR}/docker-compose.yml"
RUN_DIR="${ROOT}/.test-run"
ANALYSIS_ROOT="${COMPOSE_DIR}/analysis"
PROJECT="datorium-todo-$(date +%s)-$$"
...
step "MINT_TOKEN"
TOKEN="$(cd "${ROOT}" && go run ./cmd/mint-token \
...
step "BUILD_CLI"
BIN="${RUN_DIR}/todo-integration"
(cd "${ROOT}" && go build -o "${BIN}" ./cmd/todo-integration)
